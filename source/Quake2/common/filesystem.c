...
{
	#if defined(__RSX__)
	printf("FS_AddBinaryDirAsGameDirectory call no Sys_GetBinaryDir");
	exit(1);
	#else
...
{
	#if defined(__RSX__)
	printf("FS_AddGameDirectories call no Sys_GetBSys_GetHomeDirinaryDir");
	exit(1);
	#else
...
void FS_InitFilesystem()
{
	...
	/* Current directory. */
	#if defined(__RSX__)
	printf("FS_InitFilesystem call no Sys_GetCurrentDirectory\n");
	exit(1);
	char *currentDirectory =  "none";
	#else
